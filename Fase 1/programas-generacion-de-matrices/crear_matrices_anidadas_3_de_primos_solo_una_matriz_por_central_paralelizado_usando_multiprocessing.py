...
def buscaMatrices(nIN,nFIN,return_list):
...
"""
Para paralelizar la búsqueda de elementos primos haremos uso de la libreria multiprocessing 
- Haremos un split del volumen de elementos a tratar.
- Dividiremos el problema en subproblemas a tratar entre los diferentes núcleos de la cpu.
"""
...
if __name__=='__main__':
    ...
    while opt=='R' or opt=='r':
        #Solicitamos el valor mínimo y máximo en el que estará comprendida la búsqueda
        nin=int(input("Número inicial (nIN): "))
        nfin=int(input("Número final (nFIN): "))

        tiempo=time.perf_counter()
        # Lista de procesos que lanzaremos en paralelo
        procesos=list()
        #Consultamos el número de núcleos disponibles
        cores=os.cpu_count()
        print("Núcleos disponibles: ",cores)

        nmatrices=0
        nin_aux=nin
        nelementos=nfin-nin
        particion=int(nelementos/cores)
        nfin_aux=nin+particion
        
        # Recurso para almacenar los resultados de los procesos
        manager = multiprocessing.Manager()
        return_list= manager.list()
        return_list.append(nmatrices)

        for n in range(cores):        
            proceso=Process(target=buscaMatrices,args=(nin_aux,nfin_aux,return_list))        
            nin_aux=nfin_aux
            nfin_aux=nfin_aux+particion
        ...
        # Recogemos los resultados calculados por los procesos
        suma=return_list[0]  
        tiempo=time.perf_counter()-tiempo
        tiempo=round(tiempo,2)
        print("\nTOTAL DE MATRICES ENCONTRADAS: Entre " + str(nin) + " y " + str(nfin) + " -> " + str(suma) + " matrices anidadas de primos\n")
        print("Time: "+str(tiempo)+" \n")
        time.sleep(5)
        opt=str(input("Pulse una tecla para salir o R para repetir: "))
